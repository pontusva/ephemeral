...
// parseTTL converts a string like "1d", "3d", "7d" to time.Duration
func parseTTL(ttlStr string) (time.Duration, error) {
	...
	default:
		return 3 * 24 * time.Hour, nil // default to 3 days
	}
}
...
func Router(db *sql.DB) http.Handler {
	...
	botURLStr := os.Getenv("BOT_URL")
	if botURLStr == "" {
		botURLStr = "http://bot:3000"
	}
	botURL, err := url.Parse(botURLStr)
	if err == nil {
		proxy := httputil.NewSingleHostReverseProxy(botURL)
		mux.Handle("/bot/", http.StripPrefix("/bot", proxy))
	}
