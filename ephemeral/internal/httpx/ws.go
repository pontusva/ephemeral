...
func wsHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("[ws] Incoming connection for room: %s (Origin: %s)\n", token, r.Header.Get("Origin"))
		if token == "" {
		...
		if err != nil || !ok {
			log.Printf("[ws] Room %s not found or expired\n", token)
		...
		// Enforce max 2 participants + 1 bot
		if rh.count >= 3 {
			log.Printf("[ws] Room %s full (count: %d)\n", token, rh.count)
		...
		for {
			...
			// Persist MSG, IMG_META, IMG_CHUNK, IMG_END, VID_META, VID_CHUNK, VID_END for history replay
			if envelope.Type == "MSG" || envelope.Type == "IMG_META" || envelope.Type == "IMG_CHUNK" || envelope.Type == "IMG_END" ||
				envelope.Type == "VID_META" || envelope.Type == "VID_CHUNK" || envelope.Type == "VID_END" {
				var payload struct {
					Seq        int    `json:"seq"`
